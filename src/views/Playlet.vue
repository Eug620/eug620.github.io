 ...
 * @LastEditTime : 2025-01-19 00:00:06
 * @FilePath     : /eug620.github.io/src/views/Playlet.vue
...
<template>
    <div class="caricature flex flex-col h-full pt-2 gap-2">
        ...
        <div class="caricature-container max-w-screen-lg w-full  mx-auto overflow-scroll flex flex-col flex-1"
            :class="{ 'px-2': !searchShow.video_id }">
            <div v-if="!searchShow.name && !searchShow.book_id && !searchShow.video_id" class="text-center">请输入并回车检索
            </div>
            <!-- 目录 -->
            ...
            <div v-if="searchShow.book_id" class=" max-w-screen-lg w-full flex-1">
                <div class="border max-w-screen-lg w-full   mb-4 px-4 py-3 rounded-xl"
                    v-for="book_id in searchResults.book_id" :key="book_id.video_id"
                    @click="useSearch('video_id', book_id.video_id)">
                    {{ book_id.title }}
                </div>
            </div>
            <!-- 内容 -->
            <div v-if="searchShow.video_id">
                <div class="text-center text-lg	font-bold mt-6">{{ searchResults.video_id.title }}</div>
                <div class="text-center my-4">点击图片跳转观看(刷新跳转后的页面)</div>
                <a :href="searchResults.video_id.url" target="_blank">
                    <img class="w-60 mx-auto" :src="searchResults.video_id.pic" alt="">
                </a>
            </div>
            <div @click="useBack" v-if="!searchShow.name && (searchShow.book_id || searchShow.video_id)"
...
type SearchType = 'name' | 'book_id' | 'video_id'
const useSearch = (type: SearchType, id?: string) => {
    if (type !== 'name') {
        searchOptions[type] = id as string
    }
    if (!searchOptions[type]) return
    fetch(`https://www.hhlqilongzhu.cn/api/duanju_fanqie.php?${type}=${searchOptions[type]}`).then(async (response) => {
